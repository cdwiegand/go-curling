...
func SetupFlagArgs(ctx *curl.CurlContext, flags *flag.FlagSet) {
	...
	flags.StringArrayVarP(&ctx.Cookies, "cookie", "b", empty, "HTTP cookie, raw HTTP cookie only (use -c for cookie jar files)")
	flags.StringArrayVarP(&ctx.Data_Standard, "data", "d", empty, "HTML form data (send as-is, except for @file reference strips CR/LF), sets mime type to 'application/x-www-form-urlencoded' unless specified as a header")
	flags.StringArrayVar(&ctx.Data_Ascii, "data-ascii", empty, "Alias for -d/--data")
	flags.StringArrayVar(&ctx.Data_Encoded, "data-urlencode", empty, "HTML form data (value or @file lines are URL encoded), sets mime type to 'application/x-www-form-urlencoded' unless specified as a header")
	flags.StringArrayVar(&ctx.Data_RawAsIs, "data-raw", empty, "HTML form data (send value exactly as-is, no @file support), sets mime type to 'application/x-www-form-urlencoded' unless specified as a header")
	flags.StringArrayVar(&ctx.Data_Binary, "data-binary", empty, "HTML form data (send value exactly as-is, except for @file reference), sets mime type to 'application/x-www-form-urlencoded' unless specified as a header")
	...
	flags.StringArrayVarP(&ctx.Form_Multipart, "form", "F", empty, "HTML form data (multipart MIME), sets mime type to 'multipart/form-data' unless specified as a header")
	flags.StringArrayVar(&ctx.Form_MultipartRaw, "form-string", empty, "HTML form data (multipart MIME), exact value used, no @file or >file support")
	...
	flags.IntVar(&ctx.MaxRetries, "retry", 0, "Number of times to retry a request if it returns a transient error (or, with --retry-all-errors, any error)")
	flags.IntVar(&ctx.RetryDelaySeconds, "retry-delay", 0, "Seconds to wait between retries (see --retry)")
