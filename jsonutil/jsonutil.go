// from https://github.com/emacampolo/gomparator/blob/master/json_util.go under the MIT license
// with my modifications to add a filter callback function
package jsonutil
...
func Equal(vx, vy interface{}, skipCheckFilter func(string) bool) bool {
	...
	case map[string]interface{}:
		...
		for k, v := range x {
			...
			if !Equal(v, val2, skipCheckFilter) {
				return false
			}
		}
	...
	case []interface{}:
		...
		for _, v := range x {
			for i, v2 := range y {
				if Equal(v, v2, skipCheckFilter) && !flagged[i] {
					matches++
					flagged[i] = true
